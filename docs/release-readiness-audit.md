...
| Transcript locations config-driven, with no implicit filesystem walks | Partially verified | True for the DB path: `spotter transcripts sync` only ingests `--file`, `--transcript-root`, `--session` (resolved against configured roots), or configured `transcript_roots`, and prints usage when none is set (`src/cli.rs` `sync`, `find_configured_session`; `tests/cli_path_overrides.rs` covers path/config behavior). Not true for the DB-less `scan` family: with no `--file`/`--root` and no configured `transcript_roots`, `default_roots` in `src/scan.rs:87-104` implicitly walks `~/.claude/projects` and `~/.claude_agents/projects` (reached via `collect_targets` at `src/scan.rs:63-82`), which `README.md` also documents as scan fallback behavior |

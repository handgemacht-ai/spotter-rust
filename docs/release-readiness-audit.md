...
| Code quality gates: fmt, clippy, tests, rustdoc, doctests, deny, machete, unsafe, no production unwrap, MSRV | Verified | `.github/workflows/ci.yml`; `scripts/check-ci-workflow.py`; CI run `25960940387` is green on the audited tree `36bd1ab` |
...
| Coverage thresholds 80 percent lines and 70 percent branches | Verified | CI `coverage` job in run `25960940387`; local audit recorded 89.42 percent lines and 75.60 percent branches |
...
| Performance targets and hot-path benchmarks | Verified | `.github/workflows/ci.yml` performance job; CI run `25960940387` passed on the audited tree `36bd1ab` |
...
| Release dry run | GitHub Actions Release run `25960974041` succeeded with `publish=false` on `36bd1ab`: verify passed, all five target artifacts/checksums were produced, and publish was skipped |
